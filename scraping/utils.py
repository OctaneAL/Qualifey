...
from scraping.eures_scraping import scrap_eures, scrap_eures_counts, timezone
...
def eures_to_db(vacancies):
    source_id = Source.objects.get(name = 'EURES').id
    for keyword in vacancies:
        ...
        for country_code in vacancies[keyword]:
            ...
            print("GOT", country_id)
            cur = 0
            target = len(vacancies[keyword][country_code])
            for vacancy in vacancies[keyword][country_code]:
                cur += 1
                print(cur, '/', target)
                if Vacancy.objects.filter(job_id = vacancy['id'], jobtitle_id = jobtitle_id, country_id = country_id, timestamp = vacancy['timestamp'], source_id = source_id).exists():
                    continue
                obj = Vacancy(
                    job_id = vacancy['id'],
